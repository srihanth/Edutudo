// ECE JS — YouTube controls and card actions
let YTAPIReady_ece = false;
function onYouTubeIframeAPIReady(){ YTAPIReady_ece = true; }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  cards.forEach((card, idx) => {
    ...
    if(localStorage.getItem('ece_learned_'+vid) === '1') card.classList.add('learned');
  ...
  function toggleLearned(card){
    ...
    localStorage.setItem('ece_learned_'+vid, card.classList.contains('learned')? '1':'0');
    showToast(card.classList.contains('learned')? 'Marked learned':'Marked unlearned');
  }

  function downloadNotes(card){
    const title = card.querySelector('h3').innerText;
    const blob = new Blob([`Notes for ${title}\n\nReplace with your PDF or real notes.`], {type:'text/plain'});
  ...
  resetAll.addEventListener('click', ()=> { cards.forEach(c=>{ c.style.display=''; c.classList.remove('learned'); localStorage.removeItem('ece_learned_'+c.dataset.video); }); showToast('Reset done'); });
});
