// EE JS — same features customised for EE
let YTAPIReady_ee = false;
function onYouTubeIframeAPIReady(){ YTAPIReady_ee = true; }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  cards.forEach((card, idx) => {
    ...
    if(localStorage.getItem('ee_learned_'+vid) === '1') card.classList.add('learned');
  ...
  function toggleLearned(card){
    ...
    localStorage.setItem('ee_learned_'+vid, card.classList.contains('learned')? '1':'0');
    showToast(card.classList.contains('learned')? 'Marked learned':'Marked unlearned');
  }

  function downloadNotes(card){
    const title = card.querySelector('h3').innerText;
    const blob = new Blob([`EE Notes for ${title}\n\nReplace with your PDF.`], {type:'text/plain'});
  ...
  resetAll.addEventListener('click', ()=> { cards.forEach(c=>{ c.style.display=''; c.classList.remove('learned'); localStorage.removeItem('ee_learned_'+c.dataset.video); }); showToast('Reset done'); });
});
